...
function ConfidenceBadge({ level }: { level: ForecastConfidence }) {
  const map: Record<ForecastConfidence, { label: string; cls: string }> = {
    ...
    low:    { label: 'Düşük Güven',     cls: 'bg-gray-100 text-gray-600'    },
  }
  const cfg = map[level]
  return (
              ...
              return (
                <tr key={row.week} className={rowCls}>
                  <td className="px-4 py-2 font-semibold text-[#334155]">
                    Hafta {row.week}
                    {row.has_obligation && (
                      <span className="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-blue-100 text-blue-700">
                        Taahhüt
                      </span>
                    )}
                  </td>
