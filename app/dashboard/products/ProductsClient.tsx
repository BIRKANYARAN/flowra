...
// ── Props ─────────────────────────────────────────────────────────────────────
export interface LotCostEntry { avgCost: number; lotCount: number; currency: string }

interface Props {
  initialProducts: Product[]
  initialLotCosts: Record<string, LotCostEntry>
}
...
export default function ProductsClient({ initialProducts, initialLotCosts }: Props) {
  ...
  function openAdj(p: Product) {
    setAdjProduct(p); setAdjQty(''); setAdjType('adjustment')
    setAdjNotes(''); setAdjErr(''); setAdjIdemp(genKey())
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-xs text-gray-400">{list.length} ürün</p>
      ...
      {/* ── Global error ─────────────────────────────────────────────────── */}
      {formErr && !showForm && (
        <div className="mb-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2 flex items-center justify-between">
          <span>{formErr}</span>
          <button onClick={() => setFormErr('')} className="ml-3 text-red-400 font-bold">✕</button>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <select className="border border-gray-200 rounded-xl px-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 bg-white" {...f('cost_currency')}>
                  ...
                  <div key={p.id}
                    ...
                    <div className="col-span-3 flex justify-end gap-1">
                      <button onClick={() => openAdj(p)}
                        className="text-xs text-primary-600 hover:text-primary-800 px-2 py-1 rounded-lg hover:bg-primary-50 transition-colors font-medium">
                        ± Stok
                      </button>
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={closeAdj} disabled={adjSaving}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className={IL} value={adjType} onChange={e => setAdjType(e.target.value)} disabled={adjSaving}>
              ...
              <div>
                <label className={LAB}>
                  Miktar Değişimi
                  <span className="ml-1 text-gray-400 normal-case font-normal">(giriş için +, çıkış için −)</span>
                </label>
                <input type="number" step="0.001" placeholder="+10 veya -5" className={IL}
                  value={adjQty} onChange={e => setAdjQty(e.target.value)} disabled={adjSaving} autoFocus />
              ...
              <div>
                ...
                <input className={IL} placeholder="Hareket nedeni…" value={adjNotes}
                  onChange={e => setAdjNotes(e.target.value)} disabled={adjSaving} />
              </div>
            </div>

            {adjErr && (
              <div className="mt-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-3 py-2">{adjErr}</div>
            )}
