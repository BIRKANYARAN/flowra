...
interface RealCostMap {
  [productId: string]: { loading: boolean; value: number | null }
}
...
export default function CatalogPage() {
  ...
  const [realCosts, setRealCosts]   = useState<RealCostMap>({})
  ...
  /** Convert a TRY amount to the selected display currency */
  const toDisplayCurrency = useCallback((tryAmount: number): number => {
    if (currency === 'TRY') return tryAmount
    const rate = currency === 'USD' ? fxRates.USD : fxRates.EUR
    return rate > 0 ? tryAmount / rate : 0
  }, [currency, fxRates])

  /** Whether the selected currency can be displayed (TRY always works, others need FX rate) */
  ...
  const [costRows, setCostRows]             = useState<Array<{
    ...
    { label: 'Satın Alma', amount: '', currency: 'TRY', entry_date: new Date().toISOString().slice(0, 10) },
    { label: 'Gümrük',     amount: '', currency: 'TRY', entry_date: new Date().toISOString().slice(0, 10) },
    { label: 'Vergi',      amount: '', currency: 'TRY', entry_date: new Date().toISOString().slice(0, 10) },
    { label: 'Nakliye',    amount: '', currency: 'TRY', entry_date: new Date().toISOString().slice(0, 10) },
  ])

  // Load existing cost entries when modal opens
  const openCostCalc = useCallback(async (productId: string) => {
    ...
    setCostRows([
      { label: 'Satın Alma', amount: '', currency: 'TRY', entry_date: today },
      { label: 'Gümrük',     amount: '', currency: 'TRY', entry_date: today },
      { label: 'Vergi',      amount: '', currency: 'TRY', entry_date: today },
      { label: 'Nakliye',    amount: '', currency: 'TRY', entry_date: today },
    ])
  ...
  const load = useCallback(async () => {
    ...
    const cid = await resolveCompanyId(uid, supabase)
    setUserId(uid)
    setCompanyId(cid)

    // Catalog is a customer-facing browse view → hide deactivated products.
    // Legacy rows get `is_active = true` via the DB default, so this is safe.
    ...
    // Fetch current FX rates for currency conversion
    try {
      const fxRes = await fetch('/api/fx', { cache: 'no-store' })
      const fxData = await fxRes.json()
      if (fxRes.ok && fxData.USD > 0 && fxData.EUR > 0) {
        setFxRates({ USD: Number(fxData.USD), EUR: Number(fxData.EUR) })
      }
    } catch { /* FX fetch failure is non-fatal — TRY still works */ }

    // Fetch real costs for all products in parallel
    const costMap: RealCostMap = {}
    prods.forEach(p => { costMap[p.id] = { loading: true, value: null } })
    setRealCosts(costMap)
    ...
    const results = await Promise.allSettled(
      prods.map(async (p) => {
        ...
        return { id: p.id, cost: rpc?.real_cost ?? null }
      })
    )
    ...
    setRealCosts(prev => {
      ...
      results.forEach((r) => {
        if (r.status === 'fulfilled') {
          next[r.value.id] = { loading: false, value: r.value.cost }
        }
      })
      return next
    })
  ...
  // ── Catalog intelligence: highest margin, low margin, negative margin ────
  const catalogInsights = useMemo(() => {
    ...
    for (const p of products) {
      const rc = realCosts[p.id]
      if (rc?.loading || !rc?.value) continue
      const cost = rc.value
  ...
  const portfolioTotals = useMemo(() => {
    ...
    for (const p of filtered) {
      const rc = realCosts[p.id]
      if (rc?.loading || rc?.value == null) continue
      const qty = Number(p.stock_qty) || 0
      if (qty <= 0) continue

      const unitCost = rc.value
      const catalogPrice = getSalePrice(p) ?? 0

      totalCostTry += unitCost * qty
      totalRevenueTry += catalogPrice * qty
      productsCounted++
    }

    const totalCost = toDisplayCurrency(totalCostTry)
    const totalRevenue = toDisplayCurrency(totalRevenueTry)
    const totalProfit = totalRevenue - totalCost
    const avgMargin = totalRevenue > 0 ? (totalProfit / totalRevenue) * 100 : 0

    return { totalCost, totalRevenue, totalProfit, avgMargin, productsCounted }
  }, [filtered, realCosts, toDisplayCurrency])

  // ── Expand row to show lots ───────────────────────────────────────────────
  async function toggleExpand(productId: string) {
    ...
    if (lots[productId]) return // already loaded
  ...
  function startEdit(p: Product) {
    setEditingId(p.id)
    // Inline edit writes back to the legacy `catalog_price` column (Phase 2
     // migrates writes to `default_sale_price`). Read uses the adapter so new
     // rows with only `default_sale_price` still prefill correctly.
  ...
  async function savePrice(productId: string) {
    ...
    if (res.ok) {
      setProducts(prev =>
        prev.map(p => p.id === productId ? { ...p, catalog_price: price } : p)
      )
    }
        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            <div className="divide-y divide-gray-50">
              {filtered.map(p => {
                const rc = realCosts[p.id]
                const realCostTry = rc?.value ?? 0
                const rcLoading = rc?.loading ?? true
                const catalogPriceTry = getSalePrice(p) ?? 0

                // Convert to display currency
                const realCost = toDisplayCurrency(realCostTry)
                const catalogPrice = toDisplayCurrency(catalogPriceTry)
                const profit = catalogPrice - realCost
                const margin = catalogPrice > 0 ? (profit / catalogPrice) * 100 : 0
                      ...
                      <div className="col-span-3 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {p.sku && (
                              <div className="text-xs text-gray-400 mt-0.5">SKU: {p.sku}</div>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="col-span-2 text-right">
                        {rcLoading || catalogPrice === 0 ? (
                          ...
                          <div className="flex flex-col items-end gap-0.5">
                            ...
                            {margin < 0 && (
                              <span className="text-[10px] text-red-500 font-medium">Fiyat artır</span>
                            )}
                            {margin >= 0 && margin < 10 && (
                              <span className="text-[10px] text-amber-500 font-medium">Marj düşük</span>
                            )}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="bg-gray-50 px-5 py-4 border-t border-gray-100">
                        ...
                        {!productLots || productLots.loading ? (
                          ...
                          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                            <div className="grid grid-cols-14 text-[10px] font-bold text-gray-400 uppercase tracking-widest px-4 py-2 border-b border-gray-100" style={{ gridTemplateColumns: 'repeat(14, minmax(0, 1fr))' }}>
                              ...
                              {productLots.lots.map(lot => {
                                ...
                                return (
                                  <div key={lot.id} className="items-center px-4 py-2.5 text-sm grid" style={{ gridTemplateColumns: 'repeat(14, minmax(0, 1fr))' }}>
                                    ...
                                    <div className="col-span-2 text-right tabular-nums font-medium text-gray-800">
                                      {Number(lot.qty_remaining).toFixed(2)}
                                    </div>
                                    <div className="col-span-2 text-right tabular-nums text-gray-700">
                                      {currencySym(lot.cost_currency)}{fmt(lot.unit_cost)}
                                    </div>
                                    <div className="col-span-2 text-center text-gray-500">
                                      {lot.cost_currency}
                                    </div>
                                    <div className="col-span-2 text-right tabular-nums text-gray-500">
                                      {Number(lot.fx_rate_at_entry).toFixed(4)}
                                    </div>
                                    <div className="col-span-2 text-right tabular-nums font-medium text-gray-800">
                                      ₺{fmt(lotCostTry)}
                                    </div>
                                    <div className="col-span-2 text-right tabular-nums font-bold text-primary-700">
                                      ₺{fmt(lotValue)}
                                    </div>
                                  </div>
                                )
              ...
              <div className="grid grid-cols-12 items-center px-5 py-4 bg-gray-900 text-white rounded-b-2xl -mt-1">
                <div className="col-span-3 text-sm font-bold">
                  ...
                  <span className="text-xs text-gray-400 ml-2 font-normal">
                    ({portfolioTotals.productsCounted} ürün)
                  </span>
                </div>
                <div className="col-span-1" />
                <div className="col-span-2 text-right text-sm tabular-nums font-semibold">
                  {SYM}{fmt(portfolioTotals.totalCost)}
                </div>
                <div className="col-span-2 text-right text-sm tabular-nums font-semibold">
                  {SYM}{fmt(portfolioTotals.totalRevenue)}
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-0.5">
                  {products.find(p => p.id === costCalcId)?.name ?? ''}
                </p>
              </div>
              <button
