...
export async function SalesContent({ companyId }: Props) {
  ...
  try {
    const { data, error } = await supabase
      .from('sales')
      .select('id, customer_name, currency, total, total_try, nominal_profit, created_at, proforma_id, payment_status, shipment_status, proformas(proforma_no, deleted_at)')
