...
// ─────────────────────────────────────────────────────────────────────────────
// /dashboard/collections — Phase 6 hardened
//
// Features:
//   • Overdue section pinned to top with red accent
//   • Inline partial payment: enter amount_paid → auto-status to 'partial'
//   • Remaining balance shown for partial rows
//   • paid_at timestamp shown for paid rows
//   • Quick mark-paid, mark-overdue, undo actions
//   • Tab: Bekleyenler / Ödenenler / Tümü
// ─────────────────────────────────────────────────────────────────────────────
...
export default function CollectionsPage() {
  ...
  const load = useCallback(async (status: TabKey) => {
    ...
    try {
      ...
      setRows(await res.json().then(d => Array.isArray(d) ? d : []))
    } catch {
    ...
    <div className="max-w-5xl flex flex-col gap-4">
      ...
      {!loading && rows.length > 0 && (
        <p className="text-[10px] text-gray-400 text-center">
          Son 200 kayıt · Eski verilere Analitik sayfasından ulaşabilirsiniz.
        </p>
      )}
    </div>
  )
