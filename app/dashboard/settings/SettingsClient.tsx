...
// ── SettingsClient — full CRUD forms for company settings ────────────────────
// Receives prefetched data as props — no loading spinner on mount.
// Mutations: supabase direct (banks), fetch API (logo upload, interest rate, demo).
...
export type IntRateRow = { rate_date: string; annual_rate: number; source?: string }

// ── Pure validation helpers (tested in tests/settings-validation.test.ts) ─────

export function validateInterestRate(rateStr: string): string | null {
  if (!rateStr.trim()) return 'Faiz oranı boş olamaz.'
  const n = parseFloat(rateStr)
  if (!isFinite(n) || n < 0 || n > 1000)
    return 'Geçersiz faiz oranı (0–1000 arasında olmalıdır).'
  return null
}

export function validateBankForm(bankName: string, iban: string): string | null {
  if (!bankName.trim()) return 'Banka adı zorunludur.'
  if (!iban.trim())     return 'IBAN zorunludur.'
  return null
}
...
interface Props {
  ...
  initialBanks:      CompanyBank[]
  initialIntHistory: IntRateRow[]
}

// ── Component ─────────────────────────────────────────────────────────────────

export default function SettingsClient({ initialSettings, initialBanks, initialIntHistory }: Props) {
  ...
  // Logo state: signedPreview for <img>, logoPath stored in DB
  const initLogo = initialSettings?.logo_url ?? ''
  const [signedPreview, setSignedPreview] = useState(initLogo.startsWith('http') ? initLogo : '')
  const [logoPath,      setLogoPath]      = useState(initLogo)
  ...
  // ── Banks ─────────────────────────────────────────────────────────────────
  const [banks,      setBanks]      = useState<CompanyBank[]>(initialBanks)
  ...
  const [intHistory,  setIntHistory]  = useState<IntRateRow[]>(initialIntHistory)
  ...
  const loadIntHistory = useCallback(async (currency: 'TRY' | 'USD' | 'EUR' = intCurrency) => {
    const { data } = await supabase
      ...
      .limit(6)
    setIntHistory((data || []) as IntRateRow[])
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      ...
      if (!res.ok) { flash(setMsg, data.error || 'Yükleme hatası', 'error'); return }
  ...
  async function saveSettings() {
    ...
    setSaving(false)
    flash(setMsg,
      error ? 'Kayıt sırasında bir hata oluştu. Lütfen tekrar deneyin.' : 'Kaydedildi ✓',
      error ? 'error' : 'success'
    )
  }
  ...
  async function saveBank() {
    const err = validateBankForm(bankForm.bank_name, bankForm.iban)
    if (err) { flash(setBankMsg, err, 'error'); return }
    ...
    if (editBankId) {
      ...
      if (error) { flash(setBankMsg, 'Banka bilgileri güncellenemedi.', 'error'); setBankSaving(false); return }
    } else {
      ...
      if (error) { flash(setBankMsg, 'Banka hesabı eklenemedi.', 'error'); setBankSaving(false); return }
    }
  ...
  async function saveInterestRate() {
    const err = validateInterestRate(intRate)
    if (err) { flash(setMsg, err, 'error'); return }
    ...
    if (!res.ok) {
      flash(setMsg, json.error ?? 'Faiz oranı kaydedilemedi.', 'error')
    } else {
      flash(setMsg, `${intCurrency} faiz oranı kaydedildi ✓`)
    }
    setIntRate('')
    setIntSaving(false)
    await loadIntHistory(intCurrency)
  }
  ...
  async function resetDemo() {
    if (!confirm('Bu işlem tüm demo verileri silecek. Emin misiniz?')) return
      ...
      <div className="grid grid-cols-2 gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          <FlowraCard>
            ...
            {banks.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-4">Henüz banka hesabı eklenmedi.</p>
            ) : (
                ...
                {banks.map(b => (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-lg px-3 py-2 border transition-colors ${
                      b.is_default ? 'border-primary-200 bg-primary-50' : 'border-gray-100 hover:bg-gray-50'
                    }`}
                  >
                    ...
                    <div className="flex gap-1 ml-2 flex-shrink-0">
                      ...
                      <FlowraButton variant="ghost" size="sm" onClick={() => openEditBank(b)}>Düzenle</FlowraButton>
                      <FlowraButton variant="ghost" size="sm" onClick={() => deleteBank(b.id)}>Sil</FlowraButton>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-1 mb-3">
              {(['TRY', 'USD', 'EUR'] as const).map(c => (
                <button
                  key={c}
                  type="button"
                  onClick={() => { setIntCurrency(c); setIntRate(''); loadIntHistory(c) }}
