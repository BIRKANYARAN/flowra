...
import {
  fmt, fmtFull, pct, loadLocal, persistLocal,
  fetchDBScenarios, saveDBScenario, deleteDBScenario, markDBBaseline,
  computeScenario, MAX_SAVED, type Baseline, type SavedScenario,
} from './scenario'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="px-4 py-3 bg-brand-subtle border border-brand/10 rounded flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-bold text-brand">Senaryo Editörü — {period}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          <div className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8]">Değişkenler</div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold tabular-nums text-[#0f172a]">%{taxRateOverride}</span>
            </div>
            <input
          ...
          {showSaveBox && (
            <div className="bg-brand-subtle border border-brand/10 rounded px-3 py-3 space-y-2">
              <div className="text-[9px] font-bold uppercase tracking-wider text-brand-light">Senaryo Adı</div>
          ...
          {(scenariosLoading || saved.length > 0) && (
            <div className="space-y-1.5">
              <div className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8] flex items-center justify-between">
                  ...
                  <div key={s.id} className={`bg-white border rounded px-3 py-2.5 flex items-center justify-between gap-2 transition-colors ${
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isBaseline && (
                          <span className="flex-shrink-0 text-[8px] font-bold uppercase tracking-wide bg-brand-subtle text-brand px-1.5 py-0.5 rounded">
                            ★ Referans
                          </span>
                        )}
                      </div>
        ...
        <div className="space-y-3">
                ...
                <div className="px-3 py-2 bg-[#f8fafc]/60 border-b border-[#e8eaef] grid gap-1" style={{ gridTemplateColumns: `5rem repeat(${cols.length}, 1fr)` }}>
                  ...
                  {cols.map((s, i) => (
                    <div key={s.id} className="text-center">
                      <div className="text-[9px] font-bold text-[#334155] truncate leading-tight" title={s.name}>{s.name}</div>
                ...
                <div className="divide-y divide-[#f1f5f9]">
                  {rowDef.map(row => {
                    ...
                    return (
                      <div key={row.label} className={`grid gap-1 px-3 py-2 items-center ${row.divider ? 'border-t border-[#e8eaef] bg-[#f8fafc]/30' : ''}`} style={{ gridTemplateColumns: `5rem repeat(${cols.length}, 1fr)` }}>
                        ...
                        {vals.map((v, i) => (
                          <div key={i} className={`text-center text-[10px] font-bold tabular-nums rounded py-0.5 ${
                            i === best
                              ? row.higherIsBetter
                  ...
                  {hasRunway && (
                    <div className="grid gap-1 px-3 py-2 items-center" style={{ gridTemplateColumns: `5rem repeat(${cols.length}, 1fr)` }}>
                      ...
                      {computed.map((r, i) => (
                        <div key={i} className={`text-center text-[10px] font-bold tabular-nums ${r.runwayMonths === null ? 'text-pos-text' : r.runwayMonths < 3 ? 'text-neg' : 'text-warn-text'}`}>
          ...
          <div className="bg-white border border-[#e8eaef] rounded-xl shadow-soft overflow-hidden">
            <div className="px-4 py-2.5 bg-[#f8fafc]/60 border-b border-[#e8eaef]">
              <span className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8]">Kar/Zarar Özeti</span>
            </div>
            <div className="divide-y divide-[#f1f5f9]">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white border border-[#e8eaef] rounded-xl shadow-soft px-3 py-2.5">
              ...
              <div className={`text-lg font-bold tabular-nums ${result.grossMarginPct >= 0.25 ? 'text-pos-text' : result.grossMarginPct > 0 ? 'text-warn-text' : 'text-neg'}`}>
                {pct(result.grossMarginPct)}
              </div>
            </div>
            <div className="bg-white border border-[#e8eaef] rounded-xl shadow-soft px-3 py-2.5">
              <div className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8]">KDV Net</div>
              <div className={`text-lg font-bold tabular-nums ${result.vatNet > 0 ? 'text-warn' : 'text-pos-text'}`}>
          ...
          <div className="bg-white border border-[#e8eaef] rounded-xl shadow-soft overflow-hidden">
            <div className="px-4 py-2.5 bg-[#f8fafc]/60 border-b border-[#e8eaef]">
              <span className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8]">Dağıtım & Nakit</span>
            </div>
            <div className="px-4 py-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-[#64748b]">Tahsil edilen (bu ay)</span>
                <span className="text-[11px] font-bold tabular-nums text-[#1e293b]">₺{fmtFull(result.cashCollected)}</span>
              </div>
              {result.runwayMonths !== null && (
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-[#64748b]">Tahmini runway</span>
                  <span className={`text-[11px] font-bold tabular-nums ${result.runwayMonths < 3 ? 'text-neg' : 'text-warn-text'}`}>
              ...
              <div className="border-t border-[#f1f5f9] pt-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[11px] font-bold tabular-nums ${result.distributable > 0 ? 'text-pos-text' : 'text-[#94a3b8]'}`}>
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  <span className={`text-[12px] font-bold tabular-nums ${result.netDistrib > 0 ? 'text-pos-text' : 'text-[#94a3b8]'}`}>
          ...
          {cascadeStory && (
            <div className="bg-[#f8fafc] border border-[#e8eaef] rounded overflow-hidden">
              <div className="px-4 py-2.5 border-b border-[#e8eaef]">
                <span className="text-[9px] font-bold uppercase tracking-wider text-[#94a3b8]">Bu Senaryo Zinciri</span>
              </div>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs font-bold tabular-nums ${valueColor}`}>{display}</span>
      </div>
      <input
...
function PnlRow({
  ...
  return (
    <div className={`flex items-center justify-between px-4 py-2 ${accent ? 'bg-[#f8fafc]/60' : ''}`}
      ...
      <span className={`text-[10px] ${bold ? 'font-bold text-[#1e293b]' : 'text-[#64748b]'}`}>{label}</span>
      <div className="flex items-center gap-2">
        ...
        <span className={`text-[11px] tabular-nums ${bold ? 'font-bold' : 'font-semibold'} ${
