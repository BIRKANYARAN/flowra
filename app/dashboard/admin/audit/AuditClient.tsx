...
// ── AuditClient — filters + pagination + row expansion ───────────────────────
// Receives initialLogs + initialTotal from server component (no loading flash).
// Filter/page changes re-fetch from /api/admin/audit.
...
export default function AuditClient({ initialLogs, initialTotal }: Props) {
  ...
  // Row expansion for old_data / new_data JSON diff
  const [expanded, setExpanded] = useState<string | null>(null)

  // Filters — local controlled state (no re-fetch on every keystroke;
  // re-fetch happens on submit or individual select change)
  ...
  const load = useCallback(async (
    ...
    try {
      ...
      if (!res.ok) { setError('Audit logları yüklenemedi.'); return }
  ...
  function handleActionChange(e: ChangeEvent<HTMLSelectElement>) {
    ...
    setFilterAction(val)
    load(0, val, filterEntityType, filterSince)
  }

  function handleEntityTypeChange(e: ChangeEvent<HTMLSelectElement>) {
    const val = e.target.value
    setFilterEntityType(val)
    load(0, filterAction, val, filterSince)
  }

  function handleSinceChange(e: ChangeEvent<HTMLInputElement>) {
    const val = e.target.value ? e.target.value + 'T00:00:00Z' : ''
    setFilterSince(val)
    load(0, filterAction, filterEntityType, val)
  }
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5 flex flex-wrap gap-3 items-end">
        ...
        <div>
          ...
          <input
            type="date"
            className={SEL}
            value={filterSince ? filterSince.slice(0, 10) : ''}
          ...
          {total > PAGE_SIZE && (
            <div className="flex items-center justify-between mt-4 px-1">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => load(offset - PAGE_SIZE, filterAction, filterEntityType, filterSince)}
                  disabled={offset === 0}
