        ...
        <div className="px-5 py-4 border-t border-[#e8eaef] bg-[#f8fafc]/40">
          <div className="ml-auto w-56 space-y-1.5">
            ...
            <div className="bg-[#0f172a] text-white rounded px-4 py-3 flex justify-between mt-1">
              ...
              <span className="font-bold tabular-nums">{fmtMoney(s.total, s.currency)}</span>
            </div>
            {s.currency !== 'TRY' && (
      ...
      <div className="bg-white border border-[#e8eaef] rounded-xl shadow-soft p-5">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <div className="text-lg font-bold tabular-nums text-[#64748b]">{fmt(s.total_cost)}</div>
          </div>
          <div>
            <div className="text-xs text-[#94a3b8] mb-1">Nominal Kâr</div>
            <div className={`text-lg font-bold tabular-nums ${s.nominal_profit >= 0 ? 'text-pos-text' : 'text-neg'}`}>
              {fmt(s.nominal_profit)}
            </div>
          </div>
          <div>
            <div className="text-xs text-[#94a3b8] mb-1">Reel Kâr</div>
            <div className={`text-lg font-bold tabular-nums ${s.real_profit >= 0 ? 'text-pos-text' : 'text-neg'}`}>
              {fmt(s.real_profit)}
            </div>
            {s.interest_rate > 0 && (
