...
// ── BackupsClient — create, download, restore, JSON export/import ─────────────
// Receives initialBackups from server component (no loading flash).
// Create / restore mutations re-fetch /api/backups to refresh the list.
...
export function fmtBackupDate(name: string): string {
...
export default function BackupsClient({ initialBackups }: Props) {
  ...
  async function reload() {
    ...
    const json = await res.json() as { backups?: BackupEntry[]; error?: string }
    if (!res.ok) { setError(json.error ?? 'Yedek listesi alınamadı'); return }
  ...
  async function createBackup() {
    ...
    try {
      ...
      if (!res.ok) { setError(json.error ?? 'Yedekleme başarısız'); return }
      await reload()
    } catch {
  ...
  // ── Download individual file from Supabase Storage ───────────────────────────
  async function downloadFile(path: string, fileName: string) {
    const { data, error: dlErr } = await supabase.storage.from('backups').download(path)
    if (dlErr || !data) { setError('Dosya indirilemedi'); return }
  ...
  async function downloadJsonExport() {
    ...
    try {
      ...
      if (!res.ok) { setExportMsg('Dışa aktarma başarısız'); return }
    ...
    <div>
      ...
      {backups.length === 0 ? (
        ...
        <div className="space-y-4">
          {backups.map(backup => (
            <div key={backup.name} className="bg-white border border-gray-200 rounded-xl p-5">
              {/* Header row */}
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs text-gray-400">{backup.fileCount} dosya</span>
                    <span className="text-xs text-gray-400">{fmtSize(backup.totalSize)}</span>
                    {backup.metadata.total_rows !== undefined && (
                      <span className="text-xs text-gray-400">{backup.metadata.total_rows} kayıt</span>
                    )}
                  </div>
                </div>
              ...
              {backup.metadata.tables && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {Object.entries(backup.metadata.tables).map(([table, count]) => (
                    <span key={table} className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-lg">
      ...
      {restoreTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Geri Yükleme Onayı</h3>
            ...
            <div className="flex gap-2">
              ...
              <Btn variant="secondary" onClick={closeRestoreModal}>İptal</Btn>
            </div>
          </div>
        </div>
      )}
