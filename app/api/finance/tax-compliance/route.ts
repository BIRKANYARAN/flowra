...
//
// Returns Turkish tax compliance calendar + obligation tracker.
// Auth: admin only (tax data is sensitive).
...
import { TaxComplianceService } from '@/lib/services/finance/tax-compliance.service'
...
export async function GET(req: NextRequest) {
  ...
  if (memberRow.role !== 'admin') {
    return NextResponse.json(
      { error: 'Forbidden: admin role required for tax data', code: 'FORBIDDEN', type: 'AUTH' },
      { status: 403, headers: { [REQUEST_ID_HEADER]: ctx.requestId } },
    )
  }
