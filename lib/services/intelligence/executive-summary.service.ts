...
export class ExecutiveSummaryService {
  ...
  async getReport(companyId: string): Promise<ExecutiveSummaryReport> {
    const now      = new Date()
    const today    = now.toISOString().slice(0, 10)
    const yearStart = `${now.getFullYear()}-01-01`
    const periodKey = today.slice(0, 7)   // 'YYYY-MM'

    // ── 1. Finance: BurnRateService + direct queries ───────────────────────────
    const [burnReport, salesYtd, expensesYtd, overdueData, stockData, partnersData, loansData, commitmentsData] =
      await Promise.allSettled([
        ...
        // Overdue receivables (for commercial score)
        this.supabase
          .from('sales')
          .select('total_try, paid_amount, due_date, customer_id')
          .eq('company_id', companyId)
          .is('deleted_at', null)
        ...
        // Stock for operations
        this.supabase
          .from('stock_lots')
          .select('qty_remaining, products(id, stock_alert_qty)')
          .eq('company_id', companyId)
          .is('deleted_at', null)
          .gt('qty_remaining', 0),
        ...
        this.supabase
          .from('partners')
          .select('id, name, share_ratio, committed_equity_try, paid_equity_try')
          .eq('company_id', companyId)
          .is('deleted_at', null),

        // Active partner loans
        this.supabase
          .from('partner_loan_tranches')
          .select('partner_id, principal_try, total_repaid_try, expected_repayment_date, interest_rate_annual_pct, status')
          .eq('company_id', companyId)
          .is('deleted_at', null)
    ...
    // ── 4. Active customers count (last 90 days) ───────────────────────────────
    let activeCustomers = 0
    try {
      const ninetyDaysAgo = new Date(now.getTime() - 90 * 86_400_000).toISOString().slice(0, 10)
      ...
      for (const row of (custData ?? [])) {
        const key = String(row.customer_id ?? row.customer_name ?? 'unknown')
    ...
    // Finance
    const burnData = burnReport.status === 'fulfilled' ? burnReport.value : null
    const cashRunwayMonths = burnData?.runway_estimate_months ?? null

    const salesRows = salesYtd.status === 'fulfilled' ? (salesYtd.value.data ?? []) : []
    const revenueYtd  = salesRows.reduce((s: number, r: { total_try?: number | null }) => s + Number(r.total_try ?? 0), 0)
    const cogsYtd     = salesRows.reduce((s: number, r: { cost_of_goods_try?: number | null }) => s + Number((r as { cost_of_goods_try?: number | null }).cost_of_goods_try ?? 0), 0)

    const expRows     = expensesYtd.status === 'fulfilled' ? (expensesYtd.value.data ?? []) : []
    const expensesYtdTotal = expRows.reduce((s: number, r: { amount_try?: number | null }) => s + Number(r.amount_try ?? 0), 0)

    const grossMarginPct = revenueYtd > 0 ? ((revenueYtd - cogsYtd) / revenueYtd) * 100 : null
    const netIncomeYtd   = revenueYtd - cogsYtd - expensesYtdTotal
    const netMarginPct   = revenueYtd > 0 ? (netIncomeYtd / revenueYtd) * 100 : null

    const financeScore = computeFinanceScore(
      grossMarginPct ?? 0,
      cashRunwayMonths ?? 0,
      netMarginPct ?? 0,
    )
    ...
    const financeSummary: FinanceSummary = {
      gross_margin_pct:   grossMarginPct !== null ? round2(grossMarginPct) : null,
      net_margin_pct:     netMarginPct   !== null ? round2(netMarginPct)   : null,
      cash_runway_months: cashRunwayMonths,
      revenue_ytd_try:    round2(revenueYtd),
      expenses_ytd_try:   round2(expensesYtdTotal),
      net_income_ytd_try: round2(netIncomeYtd),
      finance_score:      financeScore,
    }

    // Commercial
    const overdueRows = overdueData.status === 'fulfilled' ? (overdueData.value.data ?? []) : []
    const overdueReceivablesTry = overdueRows.reduce((s: number, r: { total_try?: number | null; paid_amount?: number | null }) =>
      s + Math.max(0, Number(r.total_try ?? 0) - Number(r.paid_amount ?? 0)), 0)
    const overdueCount  = overdueRows.length
    const commercialScore = Math.max(0, Math.min(100, 100 - overdueCount * 5))

    const commercialSummary: CommercialSummary = {
      active_customers:        activeCustomers,
      revenue_mom_pct:         null,  // requires prior month — not blocking for MVP
      win_rate_pct:            null,  // requires proforma win/loss — not blocking
      overdue_receivables_try: round2(overdueReceivablesTry),
      overdue_count:           overdueCount,
      commercial_score:        commercialScore,
    }

    // Operations — critical stock: lots where qty_remaining <= stock_alert_qty
    const stockRows = stockData.status === 'fulfilled' ? (stockData.value.data ?? []) : []
    let criticalStockCount = 0
    for (const row of stockRows) {
      const qty   = Number(row.qty_remaining ?? 0)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const alert = Number((row as any).products?.stock_alert_qty ?? 0)
      if (alert > 0 && qty <= alert) criticalStockCount++
    }

    const operationsScore = Math.max(0, Math.min(100,
      100 - criticalStockCount * 10 - pendingOrdersCount * 2,
    ))

    // Fill rate: approximation — if no critical stock and no pending orders → 100%
    const fillRatePct = totalProducts > 0
      ? Math.max(0, Math.min(100, ((totalProducts - criticalStockCount) / totalProducts) * 100))
      : 100
    ...
    const operationsSummary: OperationsSummary = {
      ...
      fill_rate_pct:        round2(fillRatePct),
      operations_score:     operationsScore,
    }

    // Partners — use computePartnerRiskReport for grades
    const partnersRows  = partnersData.status   === 'fulfilled' ? (partnersData.value.data  ?? []) : []
    const loansRows     = loansData.status       === 'fulfilled' ? (loansData.value.data     ?? []) : []
    const commitRows    = commitmentsData.status === 'fulfilled' ? (commitmentsData.value.data ?? []) : []

    const totalPartners     = partnersRows.length
    const totalLoanBalance  = loansRows.reduce((s: number, r: { principal_try?: number | null; total_repaid_try?: number | null }) =>
      s + Math.max(0, Number(r.principal_try ?? 0) - Number(r.total_repaid_try ?? 0)), 0)

    const equityGapTry = commitRows.reduce((s: number, r: { committed_try?: number | null; paid_try?: number | null }) =>
      s + Math.max(0, Number(r.committed_try ?? 0) - Number(r.paid_try ?? 0)), 0)

    let maxRiskGrade = 'A'
    let partnersScore = 100

    if (totalPartners > 0) {
      // Build PartnerRiskInput for each partner
      const avgMonthlyRevenue = burnData
        ? burnData.avg_monthly_revenue_try
        : (revenueYtd / 12)

      // Build a map of partner commitments
      type CommitRow = { partner_id?: string | null; committed_try?: number | null; paid_try?: number | null }
      const commitByPartner = new Map<string, { committed: number; paid: number }>()
      for (const c of (commitRows as CommitRow[])) {
        const pid = String(c.partner_id ?? '')
        if (!pid) continue
        const prev = commitByPartner.get(pid) ?? { committed: 0, paid: 0 }
        commitByPartner.set(pid, {
          committed: prev.committed + Number(c.committed_try ?? 0),
          paid:      prev.paid      + Number(c.paid_try      ?? 0),
        })
      }
      ...
      // Build a map of partner loans (net outstanding)
      const loanByPartner = new Map<string, {
        net_loan: number
        monthly_repayment: number
        tranches: Array<{ principal_try: number; expected_repayment_date: string | null; interest_rate_annual_pct: number }>
      }>()
      for (const l of (loansRows as LoanRow[])) {
        const pid = String(l.partner_id ?? '')
        if (!pid) continue
        const netLoan = Math.max(0, Number(l.principal_try ?? 0) - Number(l.total_repaid_try ?? 0))
        const prev = loanByPartner.get(pid) ?? { net_loan: 0, monthly_repayment: 0, tranches: [] }
        const rate = Number(l.interest_rate_annual_pct ?? 0)
        const principal = Number(l.principal_try ?? 0)
        // Monthly repayment estimate
        const monthlyRep = rate > 0 ? netLoan * (rate / 100) / 12 : netLoan * 0.015
        loanByPartner.set(pid, {
          net_loan:          prev.net_loan + netLoan,
          monthly_repayment: prev.monthly_repayment + monthlyRep,
          tranches: [...prev.tranches, {
            principal_try:              principal,
            expected_repayment_date:    l.expected_repayment_date ?? null,
      ...
      type PartnerRow = {
        ...
        committed_equity_try?: number | null
        paid_equity_try?: number | null
      }

      const partnerInputs: PartnerRiskInput[] = (partnersRows as PartnerRow[]).map(p => {
        const pid      = String(p.id ?? '')
        const loanInfo = loanByPartner.get(pid) ?? { net_loan: 0, monthly_repayment: 0, tranches: [] }
        const commitInfo = commitByPartner.get(pid) ?? { committed: 0, paid: 0 }
        return {
          partner_id:             pid,
          partner_name:           String(p.name ?? pid),
          share_pct:              Number(p.share_ratio ?? 0) * 100,
          net_loan_try:           loanInfo.net_loan,
          committed_equity_try:   commitInfo.committed || Number(p.committed_equity_try ?? 0),
          paid_equity_try:        commitInfo.paid      || Number(p.paid_equity_try      ?? 0),
          monthly_repayment_try:  loanInfo.monthly_repayment,
          tranches:               loanInfo.tranches,
        }
      })

      const riskInput: PartnerRiskReportInput = {
        company_id:          companyId,
        partners:            partnerInputs,
        total_company_debt:  totalLoanBalance,
        avg_monthly_revenue: avgMonthlyRevenue,
      }

      const riskReport = computePartnerRiskReport(riskInput)
      const grades     = riskReport.profiles.map(p => p.grade)
      maxRiskGrade     = worstGrade(grades)
      partnersScore    = gradeToScore(maxRiskGrade)
    }

    const partnerSummary: PartnerSummary = {
      total_partners:         totalPartners,
      total_loan_balance_try: round2(totalLoanBalance),
      max_risk_grade:         maxRiskGrade,
      equity_gap_try:         round2(equityGapTry),
      partners_score:         partnersScore,
    }

    // ── 6. Compute overall score and health ────────────────────────────────────
    const overallScore  = computeOverallHealthScore(financeScore, commercialScore, operationsScore, partnersScore)
    const overallHealth = classifyOverallHealth(overallScore)

    // ── 7. Generate top_alerts and top_positives ───────────────────────────────
    const alerts: ExecutiveSummaryReport['top_alerts']    = []
    const positives: ExecutiveSummaryReport['top_positives'] = []

    // Finance alerts
    if (cashRunwayMonths !== null && cashRunwayMonths < 3) {
      alerts.push({ severity: 'critical', area: 'finance', message: `Nakit akışı kritik — runway ${cashRunwayMonths.toFixed(1)} ay` })
    } else if (cashRunwayMonths !== null && cashRunwayMonths < 6) {
      alerts.push({ severity: 'warning', area: 'finance', message: `Nakit runway ${cashRunwayMonths.toFixed(1)} aya düşmüş` })
    }

    if (netMarginPct !== null && netMarginPct < 0) {
      alerts.push({ severity: 'critical', area: 'finance', message: `Net marj negatif: %${netMarginPct.toFixed(1)}` })
    } else if (netMarginPct !== null && netMarginPct > 15) {
      positives.push({ area: 'finance', message: `Net marj güçlü: %${netMarginPct.toFixed(1)}` })
    }

    if (grossMarginPct !== null && grossMarginPct >= 40) {
      positives.push({ area: 'finance', message: `Brüt marj sağlıklı: %${grossMarginPct.toFixed(1)}` })
    } else if (grossMarginPct !== null && grossMarginPct < 20) {
      alerts.push({ severity: 'warning', area: 'finance', message: `Brüt marj düşük: %${grossMarginPct.toFixed(1)}` })
    }

    // Commercial alerts
    if (overdueCount > 10) {
      alerts.push({ severity: 'critical', area: 'commercial', message: `${overdueCount} vadesi geçmiş fatura tahsil edilemedi` })
    } else if (overdueCount > 5) {
      alerts.push({ severity: 'warning', area: 'commercial', message: `${overdueCount} vadesi geçmiş fatura var` })
    }

    if (activeCustomers >= 10) {
      positives.push({ area: 'commercial', message: `${activeCustomers} aktif müşteri (son 90 gün)` })
    }

    // Operations alerts
    if (criticalStockCount > 5) {
      alerts.push({ severity: 'critical', area: 'operations', message: `${criticalStockCount} ürün kritik stok seviyesinde` })
    } else if (criticalStockCount > 0) {
      alerts.push({ severity: 'warning', area: 'operations', message: `${criticalStockCount} ürün stok uyarı seviyesinde` })
    }

    if (criticalStockCount === 0 && totalProducts > 0) {
      positives.push({ area: 'operations', message: 'Tüm ürünler yeterli stok seviyesinde' })
    }

    // Partner alerts
    if (maxRiskGrade === 'F') {
      alerts.push({ severity: 'critical', area: 'partners', message: 'Bir veya daha fazla ortakta kritik risk (F) tespit edildi' })
    } else if (maxRiskGrade === 'D') {
      alerts.push({ severity: 'warning', area: 'partners', message: 'Bir veya daha fazla ortakta yüksek risk (D) var' })
    }

    if (equityGapTry > 0) {
      alerts.push({ severity: 'warning', area: 'partners', message: `Ödenmemiş sermaye taahhüdü: ₺${Math.round(equityGapTry).toLocaleString('tr-TR')}` })
    }

    if (totalPartners > 0 && maxRiskGrade === 'A') {
      positives.push({ area: 'partners', message: 'Tüm ortaklar düşük risk profiline (A) sahip' })
    }
    ...
    // ── 8. Build status_line ──────────────────────────────────────────────────
    const concernText   = topAlerts.length    > 0 ? topAlerts[0].message    : null
    const positiveText  = topPositives.length > 0 ? topPositives[0].message : null
    const statusLine    = buildExecutiveStatusLine(overallHealth, concernText, positiveText)
