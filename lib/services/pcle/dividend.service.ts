...
export class DividendService {
  ...
  static async calculate(
    ...
    // ── Fetch canonical net income + partners + reserves + paid-in capital ──────
    // DP-3: the distributable basis is the CANONICAL net income (revenue − COGS −
    // ALL opex − corporate tax), via FinanceService. Previously it was revenue −
    // opex (ignoring COGS), which overstated distributable profit and let companies
    // over-distribute → TTK 509 exposure.
    const { FinanceService } = await import('@/lib/services/finance.service')
    const [summary, partnersRes, reservesRes, paidCapRes] = await Promise.all([
      FinanceService.getFinancialSummary(uid, companyId, { from: ytdFrom, to: ytdTo }, undefined, undefined, supabase),
      supabase
        .from('partners')
        ...
        .is('deleted_at', null)
        .order('name'),
      // Existing legal reserves from finance events
      supabase
        ...
        .is('deleted_at', null),
      // Paid-in capital — for the TTK 519 reserve cap (20% of paid-in capital)
      supabase
    ...
    // ── Canonical YTD net income (the legal distributable basis) ───────────────
    const ytdNetIncome = round2(summary.net_after_tax_try)

    // Paid-in capital — Σ paid_try across active commitments.
    const paidInCapital = round2(
      ((paidCapRes.data ?? []) as { paid_try: number }[]).reduce((s, r) => s + Number(r.paid_try ?? 0), 0)
    )

    // ── TTK 519: general legal reserve = 5% of net income annually, offset by what
    //    is already reserved, and NO further reserve once the cumulative reserve
    //    reaches 20% of paid-in capital (the statutory ceiling). This both prevents
    //    over-reservation past the cap AND does not double-require a reserve that has
    //    already been set aside. ──────────────────────────────────────────────────
    const existingReserves = ((reservesRes.data ?? []) as { amount_try: number }[])
      .reduce((s, r) => s + Number(r.amount_try ?? 0), 0)
    const reserveCeiling        = round2(paidInCapital * 0.20)
    const capReached            = existingReserves >= reserveCeiling - 0.01
    const requiredLegalReserve  = capReached ? 0 : round2(Math.max(0, ytdNetIncome * 0.05))
