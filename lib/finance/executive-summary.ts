...
export interface ExecutiveSummary {
  ...
  pnl: {
    ...
    vatNet:            number     // >0 payable, <0 carry-forward
    corporateTax:      number
    matrah:            number
    monthlyBurn:       number
    breakEvenRevenue:  number | null
    periodCollected:   number     // period operational cash received
...
// ── Formatting (used by buildDecisions) ───────────────────────────────────────
...
// ── Decision builder ───────────────────────────────────────────────────────────
//
// Order: critical → attention → opportunity → info (max 5 cards)
...
function buildDecisions(
  ...
  // Sort: critical → attention → opportunity → info; max 5
...
// ── Default CfoMetrics (zero state when DB is unavailable) ────────────────────
...
export async function getExecutiveSummary(
  ...
  // ── Partner equalization (depends on distributable_cash) ──────────────────────
