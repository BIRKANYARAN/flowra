  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0">
        ...
        <KpiTile label="Gelir (Son 3 Ay)" value={fmtTRY(last3)} accent={CHART.info} href="/dashboard/sales" />
        <KpiTile label="Açık Teklif" value={`${pipeline.open_count} adet`}
          sub={`Toplam ${fmtCompact(pipeline.open_total)}`} accent={CHART.warn} href="/dashboard/proformas" />
        <KpiTile label="Vadesi Geçen Alacak" value={fmtTRY(k.overdue_receivable_try)}
          tone={k.overdue_receivable_try > 0 ? 'negative' : 'neutral'}
          accent={CHART.neg} href="/dashboard/collections" />
      </div>
